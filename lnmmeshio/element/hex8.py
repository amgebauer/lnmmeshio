...
class Hex8 (Element):
    ...
    def __init__(self, el_type: str, nodes: List[Node]):
        ...
        if len(nodes) != self.get_num_nodes():
            raise RuntimeError('You tried to created a HEX8 element with {0} nodes'.format(len(nodes)))
    ...
    def get_faces(self) -> List[Quad4]:
        face_node_ids = [[0, 1, 2, 3], [0, 1, 5, 4], [1, 2, 6, 5], [2, 3, 7, 6], [3, 0, 4, 7], [4, 5, 6, 7]]

        return [
            Quad4(None, [self.nodes[i] for i in nodes]) for nodes in face_node_ids
        ]
    ...
    def get_edges(self) -> List[Line2]:
        edge_node_ids = [[0, 1], [1, 2], [2, 3], [3, 0], [0, 4], [1, 5], [2, 6], [3, 7], [4, 5], [5, 6], [6, 7], [7, 4]]
        return [
