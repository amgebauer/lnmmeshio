...
class ResultDescription:
    ...
    def get_line(self):
        ...
        l.write(' QUANTITY {0} VALUE {1} TOLERANCE {2}'.format(self.quantity, self.value, self.tolerance))
        return l.getvalue()
        ...
        if dis == 'structure':
            if item is None:
                try:
                    ...
                    raise ValueError('Unknown node or element of result description: {0}'.format(line))
...
class StructureResultDescription(ResultDescription):
    ...
    def __init__(self, item, quantity, value, tolerance):
        super(StructureResultDescription, self).__init__(item, quantity, value, tolerance)
    
    def get_line(self):
        return 'STRUCTURE DIS structure {0}'.format(super(StructureResultDescription, self).get_line())