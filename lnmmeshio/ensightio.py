...
def _write_case(fstream, geofile, ele_vars_props, nodal_vars_props):
    ...
    for varname, props in ele_vars_props.items():
        fstream.write('{0} per {1}:\t{2}\t{3}\t{4}\t{5}\n'.format(
            dim_to_type[props['dim']], 'element', sets[props['timesteps']], sets[props['timesteps']],
            varname, props['filename']
            ))
    
    for varname, props in nodal_vars_props.items():
        fstream.write('{0} per {1}:\t{2}\t{3}\t{4}\t{5}\n'.format(
            dim_to_type[props['dim']], 'node', sets[props['timesteps']], sets[props['timesteps']],
            varname, props['filename']
            ))
...
def write_element_variable(fstream, dis: Discretization, varname: str, data: Dict[str, np.array], binary=True):
    if binary:
        io.ens_write_string(fstream, 'C Binary', binary=True)
    ...
    for key, edata in data.items():
        ...
        if dim is None:
            ...
            raise RuntimeError('The elemnt data of variable {0} have not the correct dimesion accross all elements'.format(varname))
        elif ts != data[key].shape[0]:
            raise RuntimeError('The elemnt data of variable {0} have not the correct num_timesteps accross all elements'.format(varname))
...
def write_node_variable(fstream, dis: Discretization, varname: str, data: np.array, binary=True):
    if binary:
        io.ens_write_string(fstream, 'C Binary', binary=True)
    ...
    if data.shape[1] != len(dis.nodes):
        raise RuntimeError('The nodal data of variable {0} does not fit'.format(varname))
