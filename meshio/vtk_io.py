...
try:
    ...
    import vtk
     ...
     raise Exception('\nmeshio requires python vtk module. install with:\n   sudo dnf install vtk-python\n')
...
def read(filetype, filename):
    ...
    def _read_cells(vtk_mesh):
        ...
        # see http://www.vtk.org/doc/release/4.2/html/vtkCellType_8h.html
        vtk_to_meshio_type = {
    ...
    mesh._points = points
    mesh._cells = cells
    mesh._point_data = point_data
    mesh._cell_data = cell_data
    mesh._field_data = field_data
    
    # retrieve surface_node_map from field map. convert key (=surface ID) to string 
    for key, value in mesh._field_data.iteritems():
        try:
            mesh._surface_node_map[int(key)] = value
        except ValueError:
            mesh._surface_node_map[key] = value
...
def write(filetype, filename, mesh):
    ...
    # add surface_node_map to field_data for output (key must be string)
    for key, value in mesh._surface_node_map.iteritems():
        mesh._field_data[repr(key)] = value
    
    vtk_mesh = _generate_vtk_mesh(mesh._points, mesh._cells)

    # add point data
    pd = vtk_mesh.GetPointData()
    for name, X in mesh._point_data.iteritems():
    ...
    for key, value in mesh._cell_data.iteritems():
