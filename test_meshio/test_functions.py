...
from lnmmeshio.functions.linear_interpolation_variable import LinearInterpolationVariable
...
class TestFunctions(unittest.TestCase):
    ...
    def test_write_linearinterpolationvariable(self):
        ...
        var = LinearInterpolationVariable('a', np.array([0.0, 0.1, 0.2]), np.array([1.0, 1.1, 1.2]))

        var.write(dummy_file)

        self.assertEqual(dummy_file.getvalue(),
    ...
    def test_write_function(self):
        ...
        fcn.add_variable(LinearInterpolationVariable('a', np.array([0.0, 1.0]), np.array([0.0, 1.0])))
        ...
        self.assertEqual(lines[2], 'VARIABLE 0 NAME a TYPE linearinterpolation NUMPOINTS 2 TIMES 0.0 1.0 VALUES 0.0 1.0')
