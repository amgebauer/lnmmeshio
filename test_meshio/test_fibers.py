...
class TestFibers(unittest.TestCase):
    ...
    def test_fibers_parse(self):
        ...
        self.assertAlmostEqual(np.linalg.norm(fibs[Fiber.TypeFiber1].fiber - np.array([0.1, 0.2, 0.3])), 0.0)
        self.assertAlmostEqual(np.linalg.norm(fibs[Fiber.TypeFiber2].fiber - np.array([0.2, 0.3, 0.4])), 0.0)
        self.assertAlmostEqual(np.linalg.norm(fibs[Fiber.TypeCir].fiber - np.array([0.3, 0.4, 0.5])), 0.0)
        self.assertAlmostEqual(np.linalg.norm(fibs[Fiber.TypeTan].fiber - np.array([0.4, 0.5, 0.6])), 0.0)
