...
class TestEles(unittest.TestCase):
    ...
    def __test_ele(self, shape, cls, nnodes, faces, facetype, edges, edgetype, test_get_xi = False):
    ...
    def _test_ele_shpfcns(self, cls, xis):
        ...
        for i, xi in enumerate(xis):
            ...
            self.assertListEqual(list(shpfcns), list(cmp_vct), 'Shapefunctions differ at {0} / xi={1}'.format(i, xi))
    ...
    def test_tri3(self):
        ...
        self._test_ele_shpfcns(Tri3, [np.array([0.0, 0.0]), np.array([1.0, 0.0]), np.array([0.0, 1.0])])
